...
def get_config():
    ...
    Prepare parameters:
        ...
        --use_wandb
            默认情况下，将日期记录到wandb服务器。否则将使用tensorboard记录数据
    ...
    parser.add_argument("--use_render", action='store_true', default=True,
                        help="by default, do not render the env during training. "
                             "If set, start render. Note: something, the environment has internal render process "
                             "which is not controlled by this hyperparam.")
    parser.add_argument("--render_episodes", type=int, default=1000, help="the number of episodes to render a given env")
    parser.add_argument("--ifi", type=float, default=0.1,
                        help="the play interval of each rendered image in saved video.")

    # pretrained parameters
    parser.add_argument("--model_dir", type=str,
                        default='/home/pipiw/PycharmProjects/mappo/onpolicy/scripts/results/MPE/simple_spread/mappo/check/run6/models',
