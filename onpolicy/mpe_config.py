...
def get_config():
    ...
    Network parameters:
        ...
        --use_orthogonal
            默认情况下，对权重使用正交初始化，对偏差使用0初始化。否则，将使用xavier uniform Inilization。
            by default True, use Orthogonal initialization for weights and 0 initialization for biases. or else, will use xavier uniform inilialization.
        --gain
            by default 0.01, use the gain # of last action layer
        --use_naive_recurrent_policy
            by default False, use the whole trajectory to calculate hidden states.
            默认情况下为False，使用整个轨迹计算隐藏状态。
        --use_recurrent_policy
            by default, use Recurrent Policy. If set, do not use. 默认情况下，使用重复策略。如果设置了，请不要使用。
        --recurrent_N <int>
    ...
    PPO parameters:
        ...
        --use_value_active_masks
            by default True, whether to mask useless data in value loss.  默认情况下为True，表示是否在价值损失中屏蔽无用数据
        --huber_delta <float>
    ...
    # use_valuenorm 默认情况下为True，使用running mean和std将奖励标准化。
    ...
    parser.add_argument("--critic_lr", type=float, default=13e-4,
                        help='critic learning rate (default: 5e-4)')
    parser.add_argument("--opti_eps", type=float, default=8e-5,
